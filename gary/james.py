...
# Create map for stitch_id to ATC
stitch_to_ATC = {}
with open("atc2stitch.csv") as csv_obj:
  next(csv_obj)
  for line in csv.reader(csv_obj, dialect="excel"):
    if line[3] not in stitch_to_ATC:
      stitch_to_ATC[line[3]] = line[0]

# Create map from drug to stitch_id
drug_to_stitch = {}
doc2 = "twosides_every20_first" + version + ".tsv"
with open(doc2) as tsv:
  next(tsv)
  for line in csv.reader(tsv, dialect="excel-tab"):
      if line[2] not in drug_to_stitch:
        drug_to_stitch[line[2]] = line[0]
      if line[3] not in drug_to_stitch:
        drug_to_stitch[line[3]] = line[1]




doc1 = "drugs_effects_twosides_first" + version + ".json"
...
nodes_final = sorted(nodes_final, key=lambda k: k['ATC'])

print nodes_final

data = {"nodes": nodes_final, "links": data['links']}
